...
@Slf4j
public class BillStatusInternetProviderStrategy implements StepStrategy {
    ...
    public boolean executeStep(Long userId, Map<String, String> metadata) {
        ...
        try {
            log.info("[BillStatusInternetProviderStrategy] - Start getting bill status for user {}", userId);
            BillStatusDto billStatus = billService.getBillStatus(userId, metadata);
            log.info("[BillStatusInternetProviderStrategy] - End bill status for user {} is {}", userId, billStatus);
